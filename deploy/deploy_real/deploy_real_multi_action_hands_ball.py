...
class Controller(Node):
    def __init__(self, config_file="g1_21dof.yaml") -> None:
        ...
        # Moved start() to end of __init__ to avoid race conditions
    ...
    def load_config(self):
        ...
        self.urdf_path = config.get("urdf_path", "")
    ...
    def load_motion_trajectories(self):
        """
    ...
    def zero_torque_state(self):
        ...
        print("Hand controls: 'o' to open, 'g' to close/grip, 'h' to stop hands.")
        ...
        if self._kb_request_state is not None:
            ...
            if requested != self.current_state:
                ...
                if requested == ControllerState.POLICY:
                    ...
                    if self.motion_trajectories is not None:
                        self.motion_frame_idx = 0
                    self._play_motion_once = False
    ...
    def policy_mode_listener(self):
        ...
        # Currently not using extra buttons for mode switching, defaulting to Walk
        pass
    ...
    def stop_hands(self):
        """Stop all hand motors"""
        if self.left_hand:
            self.left_hand.stop()
        if self.right_hand:
            self.right_hand.stop()
        self.get_logger().info("Stopping hands")
    ...
    def run(self):
        ...
        for step in range(1):
            ...
            if self.motion_trajectories is not None and self._play_motion_once:
    ...
    def obs_joint_pos(self):
        ...
        if hasattr(self, 'free_joint_indices'):
            joint_pos[self.free_joint_indices] = 0.0
        return joint_pos
    ...
    def _handle_keypress(self, ch: str):
        ...
        if key == "z":
            ...
            if 0 <= motion_idx < len(self.motion_trajectories):
                ...
                self.motion_frame_idx = 0
                self._play_motion_once = True
                print(f"[Keyboard] Playing motion {self.which_motion} (pressed '{key}')", flush=True)
            else:
